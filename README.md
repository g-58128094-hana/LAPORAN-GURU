...
import { 
  ...
  Lock, LogOut, User, BarChart, Percent, Eye, PenTool, ShieldCheck, XCircle, AlertCircle, Sparkles, MessageSquare
} from 'lucide-react';

// --- KONFIGURASI GOOGLE APPS SCRIPT ---
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzvTvA24bpLWYnPnwt97hfivE-gGlqtAjStZuUq5uZX6_WMvpO1Kqxx98ioofu0DSn9Yw/exec";

// --- GEMINI API HELPER ---
const callGeminiAPI = async (prompt, jsonMode = false) => {
  const apiKey = ""; // API Key disuntik secara automatik oleh persekitaran
  ...
  try {
    ...
    if (!response.ok) throw new Error('Ralat API Gemini');
...
export default function App() {
  ...
  const [activeModule, setActiveModule] = useState('bertugas'); 
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      // Validasi semula user dari senarai master untuk pastikan role terkini
      const freshUser = USERS.find(u => u.username === parsedUser.username);
      if (freshUser) {
          setCurrentUser(freshUser);
          localStorage.setItem('skuj_user_session', JSON.stringify(freshUser));
      } else {
  ...
  // Save data to LocalStorage (Setiap kali list berubah)
  useEffect(() => {
  ...
  const handleEditLaporan = (laporan) => {
    if (currentUser.role !== 'admin' && laporan.createdBy !== currentUser.id) {
      alert("Akses dinafikan: Hanya pencipta laporan atau Pentadbir boleh menyunting."); return;
    }
  ...
  // --- GENERIC DELETE (KEMASKINI) ---
  const handleDelete = (id, createdBy, type) => {
    // 1. Semak Kebenaran (Admin atau Pemilik Asal)
    if (currentUser.role !== 'admin' && createdBy !== currentUser.id) {
      alert("Maaf, anda tidak dibenarkan memadam laporan ini.");
      return;
    }

    // 2. Notifikasi Pengesahan
    const isConfirmed = window.confirm("Adakah anda pasti mahu memadam rekod ini?\n\nTindakan ini tidak boleh dikembalikan.");

    if (isConfirmed) {
      if (type === 'bertugas') {
        // Guna functional update untuk pastikan state terkini
        setLaporanList(prevList => prevList.filter(l => l.id !== id));
      } else {
        setKehadiranList(prevList => prevList.filter(k => k.id !== id));
      }
          ...
          <div className="px-4 py-6 sm:px-0">
            ...
            {activeModule === 'bertugas' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredLaporan.length === 0 ? <p className="text-gray-500 italic col-span-full text-center py-10">Tiada laporan dijumpai. Sila cipta laporan baru.</p> : null}
            ...
            {activeModule === 'kehadiran' && (
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredKehadiran.length === 0 ? <p className="text-gray-500 italic col-span-full text-center py-10">Tiada laporan dijumpai. Sila cipta laporan baru.</p> : null}
  ...
  return (
    <div className="min-h-screen bg-indigo-900 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-2xl p-8">
        ...
        <p className="text-center text-gray-600 mb-8">Sistem Pengurusan Sekolah 2026</p>
        ...
        <div className="mt-4 flex justify-end gap-2 border-t pt-3">
            ...
            {(currentUser.role === 'admin' || item.createdBy === currentUser.id) && (
              <>
                ...
                {/* BUTANG PADAM DENGAN PENGESAHAN */}
...
function FormEditorKehadiran({ initialData, onSave, onCancel, currentUser }) {
  ...
  const handleAIAnalysis = async () => {
    setIsAnalyzing(true);
    const dataString = JSON.stringify(formData.kelasData);
    const prompt = `
      Analisis data kehadiran sekolah ini. 
      Kenal pasti kelas dengan kehadiran purata terbaik dan terburuk sepanjang minggu.
      Berikan trend ringkas jika ada penurunan kehadiran pada hari tertentu.
      Berikan respon dalam Bahasa Melayu yang ringkas dan profesional (maksimum 50 patah perkataan).
    ...
    if (analysis) {
        alert("📊 ANALISIS GEMINI AI:\n\n" + analysis);
    } else {
        alert("Gagal menjana analisis. Sila cuba lagi.");
    }
    ...
    <div className="bg-white shadow rounded-lg p-6">
       ...
       <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }}>
         ...
         <div className="flex justify-between items-end border-b border-gray-200 mb-4 pb-2">
            ...
            {/* AI BUTTON FOR ATTENDANCE */}
            <button 
                type="button" 
                onClick={handleAIAnalysis}
                disabled={isAnalyzing}
                className="mb-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-3 py-1.5 rounded-full shadow-md text-xs font-bold flex items-center gap-1.5 transition transform hover:scale-105"
            >
                {isAnalyzing ? <Loader size={14} className="animate-spin" /> : <Sparkles size={14} />}
                {isAnalyzing ? "Menganalisis..." : "Analisis Data AI"}
            </button>
         </div>
...
function FormEditorBertugas({ initialData, onSave, onCancel, currentUser }) {
  ...
  const handleGenerateSummary = async () => {
    ...
    // 1. Prepare Data
    const dailyLogs = JSON.stringify(formData.laporanHarian);
    
    // 2. Prompt Engineering
    const prompt = `
      Anda adalah pembantu AI untuk sekolah. Berdasarkan laporan harian berikut, sila bina satu RUMUSAN MINGGUAN yang profesional.
      
      Arahan Penting:
      1. Sila berikan output dalam format JSON yang SAH sahaja.
      2. Jangan gunakan markdown formatting (seperti \`\`\`json).
      3. Format JSON mestilah: { "isu": "teks...", "tindakan": "teks...", "cadangan": "teks..." }
    ...
    if (rawResult) {
      try {
        // Clean up markdown code blocks if AI accidentally adds them
        ...
        setFormData(prev => ({
          ...
          rumusan: {
            ...prev.rumusan,
            isu: result.isu || prev.rumusan.isu,
            tindakan: result.tindakan || prev.rumusan.tindakan,
            cadangan: result.cadangan || prev.rumusan.cadangan
          }
        }));
        alert("✨ Rumusan berjaya dijana oleh Gemini AI!");
      } catch (e) {
        console.error("JSON Parse Error", e);
        alert("Gagal memproses respons AI. Sila cuba lagi.");
      }
    } else {
      alert("Gagal menyambung ke Gemini AI.");
    }
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="space-y-6">
         ...
         <div className="border-t pt-4">
             <div className="flex justify-between items-center mb-2">
                 <h3 className="font-bold">Rumusan</h3>
                 {/* AI BUTTON FOR SUMMARY */}
                 <button 
                   type="button" 
                   onClick={handleGenerateSummary} 
                   disabled={isGenerating}
                   className="bg-indigo-100 hover:bg-indigo-200 text-indigo-800 px-3 py-1 rounded text-xs font-bold flex items-center gap-1 transition"
                 >
                   {isGenerating ? <Loader size={14} className="animate-spin" /> : <Sparkles size={14} />}
                   {isGenerating ? "Menjana..." : "Jana Rumusan AI"}
                 </button>
             </div>
